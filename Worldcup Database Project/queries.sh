...
echo "$($PSQL "SELECT CONCAT(year,'|', name) FROM teams JOIN games on teams.team_id = games.winner_id WHERE round='Final' ORDER BY year")"

echo -e "\nList of teams that start with 'Co':"
echo "$($PSQL "SELECT name FROM teams JOIN games on teams.team_id = games.winner_id WHERE name LIKE 'Co%' ORDER BY name")"
